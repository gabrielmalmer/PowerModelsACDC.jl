...
result = solve_acdcopf("case5_acdc.m", ACPPowerModel, Ipopt.Optimizer)
...
```julia
result = solve_acdcopf("case5_acdc.m", ACPPowerModel, Ipopt.Optimizer, setting = Dict("output" => Dict("branch_flows" => true)))
...
```julia
result = solve_ac_opf("case5_acdc.m", Ipopt.Optimizer)
```
